...
class WalkForwardTest:
    ...
    def compare_performance(self, in_sample_results, out_sample_results):
        ...
        in_sample_metrics = in_sample_results.get('performance_metrics', {})
        out_sample_metrics = out_sample_results.get('performance_metrics', {})
    ...
    def plot_equity_curves(self, in_sample_results, out_sample_results):
        ...
        in_sample_equity = in_sample_results.get('equity_curve', pd.DataFrame())
        out_sample_equity = out_sample_results.get('equity_curve', pd.DataFrame())
        ...
        in_sample_equity.index = pd.to_datetime(in_sample_equity.index)
        out_sample_equity.index = pd.to_datetime(out_sample_equity.index)
        
        # Normalize equity curves to start at 100
        in_sample_equity_norm = 100 * (in_sample_equity / in_sample_equity.iloc[0])
        out_sample_equity_norm = 100 * (out_sample_equity / out_sample_equity.iloc[0])
        
        # Create plot
        plt.figure(figsize=(12, 8))
        plt.plot(in_sample_equity_norm, label='In-Sample', color='blue')
        plt.plot(out_sample_equity_norm, label='Out-of-Sample', color='red')
        ...
        plt.title(f'{self.strategy_name} Walk-Forward Test: Equity Curves')
        plt.xlabel('Date')
        plt.ylabel('Normalized Equity (Starting at 100)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        # Save plot
        plot_path = os.path.join(self.output_dir, 'equity_curves_comparison.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Equity curves plot saved to {plot_path}")
