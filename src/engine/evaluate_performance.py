...
def evaluate_performance(results_path):
    ...
    Args:
        results_path (str): Path to the backtest_results.pkl file or its containing directory.
    """
    # If results_path is a directory, append the expected filename
    if os.path.isdir(results_path):
        results_path = os.path.join(results_path, 'backtest_results.pkl')
    ...
    if not os.path.exists(equity_curve_path):
        ...
        # Try to find the equity curve file by traversing subdirectories
        for root, dirs, files in os.walk(results_dir):
            if 'equity_curve.csv' in files:
                equity_curve_path = os.path.join(root, 'equity_curve.csv')
                print(f"Found equity curve file at {equity_curve_path}")
                break
        else:
    ...
    # Try multiple possible locations for the stock data file
    possible_paths = [
        'input/stock_data.csv',                    # Current working directory
        '../input/stock_data.csv',                 # One level up
        '../../input/stock_data.csv',              # Two levels up
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'input', 'stock_data.csv')  # Absolute path based on this file
    ]
    
    stock_data_found = False
    for stock_data_path in possible_paths:
