...
def run_monte_carlo_workflow(
    ...
    **kwargs
    ...
    if n_simulations <= 0:
        error_msg = f"Number of simulations must be positive, got {n_simulations}"
        logger.error(error_msg)
        return {
            "status": "error",
            "message": error_msg
    ...
    if not 0 < confidence_level < 1:
        error_msg = f"Confidence level must be between 0 and 1, got {confidence_level}"
        logger.error(error_msg)
        return {
            "status": "error",
            "message": error_msg
    ...
    if not 0 < bootstrap_pct <= 1:
        error_msg = f"Bootstrap percentage must be between 0 and 1, got {bootstrap_pct}"
        logger.error(error_msg)
        return {
            "status": "error",
            "message": error_msg
    ...
    if commission < 0:
        error_msg = f"Commission cannot be negative, got {commission}"
        logger.error(error_msg)
        return {
            "status": "error",
            "message": error_msg
    ...
    config = WorkflowConfig.from_kwargs(
        ...
        n_simulations=n_simulations, keep_permuted_data=keep_permuted_data,
        analyze_only=analyze_only, backtest_result=backtest_result,
        confidence_level=confidence_level, bootstrap_pct=bootstrap_pct,
        random_seed=random_seed,
        _temp_files_to_cleanup=_temp_files_to_cleanup or [], **kwargs
    )
    try:
        workflow_setup(config, "monte_carlo")
    ...
    if not parameters:
        if not param_file:
            ...
            if not param_file:
                ...
                logger.error(error_msg)
                workflow_teardown(config, "monte_carlo", None, error=Exception(error_msg))
                return {
                    "status": "error",
                    "message": error_msg,
    ...
    try:
        ...
        # Perform health check and enhanced analysis before proceeding with Monte Carlo
        print_section("Strategy Health Check & Enhanced Analysis")
        health_report = None
        enhanced_analysis = None
        
        try:
            from src.utils.strategy_health_monitor import StrategyHealthMonitor
            from src.utils.enhanced_performance_metrics import EnhancedPerformanceAnalyzer
            
            # Basic health check
            health_monitor = StrategyHealthMonitor()
            health_report = health_monitor.check_strategy_health(
                strategy_name=strategy_name,
                backtest_results=backtest_result.get('metrics', {})
            )
            
            # Enhanced performance analysis
            try:
                # Load equity curve and trade log
                equity_curve_file = os.path.join(output_dir, "backtest", "equity_curve.csv")
                trade_log_file = os.path.join(output_dir, "backtest", "trade_log.csv")
                
                if os.path.exists(equity_curve_file) and os.path.exists(trade_log_file):
                    equity_df = pd.read_csv(equity_curve_file)
                    trade_df = pd.read_csv(trade_log_file)
                    
                    # Create enhanced analyzer
                    enhanced_analyzer = EnhancedPerformanceAnalyzer(equity_df, trade_df)
                    enhanced_analysis = enhanced_analyzer.diagnose_strategy_issues()
                    
                    # Generate and save enhanced report
                    enhanced_report_text = enhanced_analyzer.generate_enhanced_report()
                    enhanced_report_path = os.path.join(output_dir, "enhanced_analysis_report.txt")
                    with open(enhanced_report_path, 'w') as f:
                        f.write(enhanced_report_text)
                    logger.info(f"Enhanced analysis report saved to: {enhanced_report_path}")
                    ...
                    if enhanced_analysis['issues_found']:
                        print(f"\n🔍 ISSUES IDENTIFIED ({len(enhanced_analysis['issues_found'])}):")
                        for issue in enhanced_analysis['issues_found']:
                    ...
                    logger.warning("Enhanced analysis skipped: equity curve or trade log not found")
            ...
            print("=" * 60)
            print(health_monitor.generate_health_report_text(health_report))
            
            # Save health report to file
            health_report_path = os.path.join(output_dir, "health_report.txt")
            with open(health_report_path, 'w') as f:
                f.write(health_monitor.generate_health_report_text(health_report))
            logger.info(f"Health report saved to: {health_report_path}")
            
            # Check if we should proceed with Monte Carlo
            should_proceed, reason = health_monitor.should_run_monte_carlo(health_report)
            
            if not should_proceed:
                logger.warning(f"Skipping Monte Carlo analysis: {reason}")
                print(f"\n⚠️  MONTE CARLO SKIPPED: {reason}")
                
                # Still return success but with warning
                return {
                    "status": "warning",
                    "message": f"Backtest completed but Monte Carlo skipped: {reason}",
                    "output_dir": output_dir,
                    "health_report": health_report,
                    "enhanced_analysis": enhanced_analysis
                }
            elif health_report['status'] == 'warning':
                logger.warning(f"Proceeding with Monte Carlo despite warnings: {reason}")
                print(f"\n⚡ PROCEEDING WITH WARNINGS: {reason}")
                
        except Exception as e:
            logger.warning(f"Health check failed, proceeding anyway: {str(e)}")
        ...
        try:
            ...
            # Generate enhanced Monte Carlo plots and visualizations
            equity_values = mc_analyzer.equity_values
            plot_files = {}
            
            logger.debug("Always generating enhanced Monte Carlo visualizations")
            ...
            if (len(equity_values) > 1) and (equity_values.iloc[0] != equity_values.iloc[-1]):
                ...
                # Enhanced plots using the MonteCarloVisualizer class (always generated)
                ...
                # Create plots based on workflow configuration
                # Default to basic plots only for performance
                plot_types = kwargs.get('monte_carlo_plot_types', ['simulation_paths'])
                enhanced_plot_files = visualizer.create_all_plots(save=True, plot_types=plot_types)
                ...
                logger.debug(f"Skipping plot generation: data_points={len(equity_values) if equity_values is not None else 0}, price_movement={equity_values.iloc[0] != equity_values.iloc[-1] if equity_values is not None and len(equity_values) > 1 else False}")
            ...
            if verbose:
                logger.exception("Full error traceback:")
            workflow_teardown(config, "monte_carlo", None, error=e)
            return {
                "status": "error",
                "message": error_msg,
                "output_dir": output_dir
    ...
    log_errors = workflow_teardown(config, "monte_carlo", workflow_result,
                                   additional_info={
