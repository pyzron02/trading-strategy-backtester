...
class MonteCarloVisualizer:
    ...
    def __init__(
        ...
        self._cache = {}
        self._precompute_expensive_calculations()
    
    def _precompute_expensive_calculations(self):
        """
        Pre-compute expensive calculations and cache them.
        """
        import logging
        logger = logging.getLogger(__name__)
        
        try:
            # Cache confidence interval bounds
            if self.simulated_paths is not None and len(self.simulated_paths.columns) > 0:
                logger.debug("Caching confidence interval calculations...")
                self._cache['lower_bound'] = self.simulated_paths.quantile((1 - self.confidence_level) / 2, axis=1)
                self._cache['upper_bound'] = self.simulated_paths.quantile(1 - (1 - self.confidence_level) / 2, axis=1)
                self._cache['median'] = self.simulated_paths.median(axis=1)
                
                # Cache return calculations
                logger.debug("Caching return calculations...")
                ...
                self._cache['returns'] = (final_equity_values / initial_equity) - 1
                
                # Pre-compute histogram data for returns
                self._cache['return_histogram'] = np.histogram(self._cache['returns'], bins=30)
                
                # Cache drawdown calculations (sample for performance)
                logger.debug("Caching drawdown calculations...")
                # Only calculate drawdowns for a sample of paths to improve performance
                ...
                self._cache['all_max_drawdowns'] = all_max_drawdowns
                self._cache['mean_max_drawdown'] = np.mean(all_max_drawdowns)
                self._cache['median_max_drawdown'] = np.median(all_max_drawdowns)
                self._cache['worst_drawdown'] = np.min(all_max_drawdowns)
                
                # Pre-compute histogram data for drawdowns
                self._cache['drawdown_histogram'] = np.histogram(all_max_drawdowns, bins=30)
                
                # Cache original equity drawdown
                original_drawdowns = self._calculate_drawdowns(pd.Series(self.equity_values))
                self._cache['original_max_drawdown'] = original_drawdowns.min()
                
                logger.debug("Finished caching expensive calculations")
            
        except Exception as e:
            logger.warning(f"Error in pre-computation: {e}. Will compute values on demand.")
            self._cache = {}

    def create_all_plots(self, save: bool = True, plot_types: List[str] = None) -> Dict[str, str]:
        """
        Create all available plot types.
        
        Args:
            save: Whether to save the plots to disk
            plot_types: List of plot types to create. If None, creates only basic plots.
                       Options: 'simulation_paths', 'return_distribution', 'drawdown_analysis', 'dashboard'
        ...
        # Default to creating all core visualizations for comprehensive analysis
        if plot_types is None:
            plot_types = ['simulation_paths', 'return_distribution', 'drawdown_analysis', 'dashboard']
        
        # Create requested plots
        if 'simulation_paths' in plot_types:
        ...
        if 'drawdown_analysis' in plot_types:
    ...
    def plot_simulation_paths(self, save: bool = True) -> Optional[str]:
        ...
        # Plot confidence interval (use cached values)
        lower_bound = self._cache.get('lower_bound', self.simulated_paths.quantile((1 - self.confidence_level) / 2, axis=1))
        upper_bound = self._cache.get('upper_bound', self.simulated_paths.quantile(1 - (1 - self.confidence_level) / 2, axis=1))
        median = self._cache.get('median', self.simulated_paths.median(axis=1))
        ...
        fig.update_layout(
            title=dict(
                text=f'{self.strategy_name}: Monte Carlo Simulation Paths<br>' +
                     '<sub>Shows potential equity curves based on random resampling of returns</sub>',
                x=0.5,
                xanchor='center'
            ),
    ...
    def plot_return_distribution(self, save: bool = True) -> Optional[str]:
        ...
        # Use cached returns or calculate if not available
        returns = self._cache.get('returns')
        if returns is None:
        ...
        # Use cached histogram data or calculate if not available
        cached_histogram = self._cache.get('return_histogram')
        if cached_histogram is not None:
            bin_values, bin_edges = cached_histogram
        else:
        ...
        fig.update_layout(
            title=dict(
                text=f'{self.strategy_name}: Return Distribution<br>' +
                     '<sub>Distribution of final returns across all Monte Carlo simulations</sub>',
                x=0.5,
                xanchor='center'
            ),
            xaxis_title='Return',
    ...
    def plot_drawdown_analysis(self, save: bool = True) -> Optional[str]:
        ...
        # Use cached drawdown calculations or calculate if not available
        all_max_drawdowns = self._cache.get('all_max_drawdowns')
        original_max_drawdown = self._cache.get('original_max_drawdown')
        
        if all_max_drawdowns is None or original_max_drawdown is None:
            all_max_drawdowns = []
            try:
        ...
        # Use cached statistics or calculate if not available
        mean_max_drawdown = self._cache.get('mean_max_drawdown', np.mean(all_max_drawdowns))
        median_max_drawdown = self._cache.get('median_max_drawdown', np.median(all_max_drawdowns))
        worst_drawdown = self._cache.get('worst_drawdown', np.min(all_max_drawdowns))
        
        # Use cached histogram data or calculate if not available
        cached_histogram = self._cache.get('drawdown_histogram')
        if cached_histogram is not None:
            bin_values, bin_edges = cached_histogram
        else:
        ...
        fig.update_layout(
            title=dict(
                text=f'{self.strategy_name}: Maximum Drawdown Distribution<br>' +
                     '<sub>Distribution of worst drawdowns across all simulations - helps assess downside risk</sub>',
                x=0.5,
                xanchor='center'
            ),
